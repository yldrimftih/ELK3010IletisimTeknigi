...
prc = t2.*t1./(t3);%raised cosine filtresinin birim vurus tepkesi
dktr=conv(dkt,prc);
qktr=conv(qkt,prc);
figure(4);
subplot(4,1,1);
...
title('Kare Darbe Kullanimda Genlik Spektrumu');
subplot(2,1,2);
Xd2 = fft(dktr);
plot(fftshift(abs(Xd2)/length(Xd2)));
title('Raised Cosine Kullanimda Genlik Spektrumu');